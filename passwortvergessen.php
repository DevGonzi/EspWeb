<?php #header('Location: not_available.php'); ?>
...
if(isset($_GET['send']) ) {
	if(!isset($_POST['email']) || empty($_POST['email'])) {
		...
		if($user === false) {
			...
			$betreff = "Neues Passwort für deinen Account auf Davids Website";
			$from = "From: Vorname Nachname <no-reply@derjoswig.de>";
...
für deinen Account auf Davids Website wurde nach einem neuen Passwort gefragt. Um ein neues Passwort zu vergeben, rufe innerhalb der nächsten 24 Stunden die folgende Website auf:
'.$url_passwortcode.'
